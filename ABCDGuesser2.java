...
public final class ABCDGuesser2 {
    ...
    //new method that takes the relative error and the minimum relative error
    //thats required to satisfy the project requirements
    //this method uses an if statement that checks if the relative error of the
    //the approximation calculated in main is small enough to be satisfied
    //if it is the method returns true otherwise it returns false
    //this boolean value will then be used in for loops for approximation output
    private static boolean checkRelError(double relError, double min) {
        //conditional to check if relative error is small enough
    ...
    public static void main(String[] args) {
        ...
        double[] charmNums = { -5, -4, -3, -2, -1, (double) -1 / 2,
                (double) -1 / 3, (double) -1 / 4, 0, (double) 1 / 4,
                (double) 1 / 3, (double) 1 / 2, 1, 2, 3, 4, 5 };
        //initializing variables to be used to increment and decrement through
        ...
        out.println(
                "The numbers you choose to be used in the jager formula calculations are "
                        ...
                        + fourthNum);
        // for loops keeps looping through until an approximation is finally derived thats
        ...
        for (minW = 0; minW < charmNums.length; minW++) {
            minX = 0;
            for (minX = 0; minX < charmNums.length; minX++) {
                minY = 0;
                for (minY = 0; minY < charmNums.length; minY++) {
                    minZ = 0;
                    for (minZ = 0; minZ < charmNums.length; minZ++) {
                        exp1 = charmNums[minW];
                        exp2 = charmNums[minX];
                        exp3 = charmNums[minY];
                        exp4 = charmNums[minZ];
                        ...
                        double min = .1;
                        //calling new method to be used for condtional to see
                        //if approx is within desired relative error
                        boolean checking = checkRelError(relError, min);
                        //if rel error method returns true, then outputs the exponents
                        //used to get there and the approx and error and then ends
                        if (checking == true) {
